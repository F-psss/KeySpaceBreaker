...
public:
    ...
    PeerSession(asio::ip::tcp::socket socket, CoordinatorServer &server, bool is_initiator);

   
    void start();


    asio::awaitable<void> send_hello(int my_id, int my_role, json_protocol::MessageType type);
    ...
    int peer_role() const { return m_peer_role; }
    ...
    void set_peer_role(int role) { m_peer_role = role; }
