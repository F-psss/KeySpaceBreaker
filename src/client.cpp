...
asio::awaitable<void> read_from_server(
    std::shared_ptr<JsonProtocol::Connection> conn
) {
    char buffer[1024];

    while (conn->is_open()) {
        JsonProtocol::Message response = co_await conn->read_message();

        std::cout << response.payload["fib"] << std::endl;

        // Выполнение какой-то нужной задачи
        int prev_fib = response.payload["prev_fib"];
        int fib = response.payload["fib"];
        int temp = fib;
        fib += prev_fib;
        prev_fib = temp;

        JsonProtocol::Message request = JsonProtocol::Message::create_response(
            response.action, {{"fib", fib}, {"prev_fib", prev_fib}}
        );
    ...
    try {
        while (conn->is_open()) {
            ...
            if (last_command == "start") {
                ...
                int b{};
                if (iss >> a >> b) {
                    JsonProtocol::Message request =
                        JsonProtocol::Message::create_response(
                            "Fibb", {{"fib", a}, {"prev_fib", b}}
                        );
                    co_await conn->send_message(request);
                } else {
...
asio::awaitable<void> client() {
    try {
        ...
        auto conn =
            std::make_shared<JsonProtocol::Connection>(std::move(socket));
